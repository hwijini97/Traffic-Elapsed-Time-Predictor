...
data_dim = 6  # 학습용 데이터의 개수(월, 일, 휴일 여부, 요일 코드, 시간대, 강수량, 통행 소요 시간)
...
y = []
for i in range(0, len(norm_elapsed_time) - output_dim):
        y.append([norm_elapsed_time[i], norm_elapsed_time[i+1], norm_elapsed_time[i+2], norm_elapsed_time[i+3],
                  norm_elapsed_time[i+4], norm_elapsed_time[i+5], norm_elapsed_time[i+6], norm_elapsed_time[i+7],
                  norm_elapsed_time[i+8], norm_elapsed_time[i+9], norm_elapsed_time[i+10], norm_elapsed_time[i+11],
                  norm_elapsed_time[i+12], norm_elapsed_time[i+13], norm_elapsed_time[i+14], norm_elapsed_time[i+15],
                  norm_elapsed_time[i+16], norm_elapsed_time[i+17], norm_elapsed_time[i+18], norm_elapsed_time[i+19],
...
# dataX는 월, 일, 휴일 여부, 요일 코드, 시간대, 통행 소요 시간
# dataY는 통행 소요 시간
...
with tf.Session() as sess:
    ...
    for i in range(iterations):
        ...
        print("{},{}".format(i, rmse_val))
    t_end = time.time()
    print("학습 소요 시간 : " + str(t_end - t_start) + "초, RMSE : " + str(rmse_val))
    print(fileName, "도로의 최소 소요시간 :", np.min(original_elapsed_time), "초, 최대 소요시간 :", np.max(original_elapsed_time), "초, 평균 소요시간 :", np.mean(original_elapsed_time), "초")
    ...
    for i in range(0, output_dim):
        ...
        print((t, int(round(reverse_min_max_scaling(original_elapsed_time, test_predict[0][i]))), round(congestion, 2)))
